...
import type { AdamTutorLevel } from './tutor-law.types';
import {
...
export function buildTrapRedirect(
  ...
  if (writingType === WritingType.KARANGAN) {
    const genre = detectKaranganGenre(topicText ?? '');
    return buildKaranganTrapRedirect(lang, genre);
  }
...
export function buildIdeationProbe(
  writingType: WritingType,
  lang: LanguageVariant,
  topicText?: string,
  profileLevel?: AdamTutorLevel,
): string {
  const isBm = lang !== LanguageVariant.ENGLISH;

  if (writingType === WritingType.KARANGAN) {
    const tier = resolveKaranganExamTier(profileLevel, topicText);
    return buildKaranganBrainstormProbe(lang, tier);
  }
...
export function buildScaffoldPrompt(
  ...
  if (writingType === WritingType.KARANGAN) {
    const genre = detectKaranganGenre(topicText ?? '');
    const tier = resolveKaranganExamTier(profileLevel, topicText);
    return buildKaranganStructureScaffold(genre, lang, tier);
  }
