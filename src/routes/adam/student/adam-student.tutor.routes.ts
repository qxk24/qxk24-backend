...
router.post('/tutor/chat', requireStudent, requireTutorSubscription, zValidator('json', TutorChatSchema), async (c) => {
  ...
  const tutorAccess = getTutorSubscriptionAccess(c);
  ...
  return stream(c, async (s) => {
    try {
      const runFreemium = isFreemiumEnabled() && Boolean(message) && Boolean(tutorAccess?.freemium);
      ...
      await withSseKeepalive(s, () =>
        streamADAMChat(
          ...
          { answerStyle: body.answerStyle, tutorProfile: body.tutorProfile, viaVoice: body.viaVoice, responseMs: body.responseMs },
        ),
      );
    } catch (err: unknown) {
